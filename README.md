...
export CORS_ORIGIN=*
# 默认保留 HTTP；如已配置 HTTPS 反向代理，可按需改为 true
...
GET  /api/auth/encryption-key  # 获取一次性认证公钥与挑战值
POST /api/auth/register        # 注册  Body: { encryptedAuth: { keyId, encryptedKey, iv, ciphertext, ... } }
POST /api/auth/login           # 登录  Body: { encryptedAuth: { keyId, encryptedKey, iv, ciphertext, ... } }
...
- 修复侧边栏切换后的页面外框闪现：加载卡片与正式内容使用独立元素，避免边框、阴影、背景串入页面动画。
- 增加桌面与移动端逐帧切换回归，保留键盘导航焦点。
- 数据库、API、备份格式不变，原有数据目录继续使用；Android versionCode 13，UGOS Pro 1.4.6.0015。
...
- **安装配置**：同步修正英文邀请码提示；新增中英文数据文件夹选择，可在安装时指定保存位置。
- **升级兼容**：文件夹留空沿用原来的数据目录；换目录前停用应用并复制数据，或导入完整备份。安装与迁移步骤见 [绿联应用包说明](ugreen/README.md#安装与数据文件夹)。
- **版本**：Web / Docker / Android `1.4.3`，Android `versionCode 10`，UGOS Pro `1.4.3.0012`。数据库结构、备份格式及应用 ID 保持不变。
...
- **应用视觉**：更新网页、UGOS Pro 和 Android 图标及启动画面；替换 PC 和移动端详情图各三张。
- **上架资料**：按现有功能重写中英文描述，提供独立的上架素材压缩包；详情原图随镜像包含在 UPK 中。
- **升级兼容**：保持数据库结构、备份格式、应用 ID 和数据挂载路径不变；保留旧版本镜像，Android 沿用已有签名。
- **版本**：`1.4.1`，Android `versionCode 8`，UGOS Pro `1.4.1.0010`。
...
- **账单日期拆分**: 账单新增“预计收款日期”和“实际结算日期”；点击已结算时记录当天的本地日期，并保留原预计收款日期。
- **通知与周期报告**: 商单到期提醒开关正式生效，通知中心新增账单逾期提醒和每周/每月数据汇总，报告频率可持久化保存。
- **WebDAV 自动备份**: 自动备份调整为应用级任务，应用打开时可在任意页面按周期执行；配置、会话密码和上次同步时间均按用户隔离。
- **备份与导入增强**: 备份增加版本号和导出时间；导入前展示数据结构、记录数和冲突预检，自动重新映射与其他用户冲突的记录 ID、商单号及关联数据，导入后立即刷新前端状态。
- **API Key 多用户隔离**: 修复 API Key 重复与串号问题，历史重复 Key 会自动换新，并增加数据库唯一约束。
- **品牌重命名修复**: 品牌重命名后同步更新商单、账单、待办和素材中的关联名称，并防止同一用户创建重名品牌。
...
- **HTTP 兼容与敏感认证加密**: 推翻“禁用 HTTP”的错误实现，恢复网页端与 Android App 的 HTTP 页面、服务地址、注册、登录和全部业务功能；登录、注册、修改邮箱/密码及账号注销改用 RSA-OAEP-256 与 AES-256-GCM 混合加密，请求体不再出现邮箱、密码和邀请码明文；增加两分钟有效的一次性挑战值，密文不可重放，并从 JWT 与认证响应中移除邮箱字段。HTTPS 仍用于抵御主动中间人篡改。
- **安全与测试**: 加强金额、日期、资产状态、待办内容和 HTTP/HTTPS 发布链接校验，升级 Capacitor 及多项依赖安全补丁；新增 15 项回归测试，并通过 TypeScript 检查、前端构建和服务端构建。
- **Android 与 Release 更新**: Android `versionCode` 升级为 `5`，重新同步最新 Web 资源并构建 APK；GitHub Release 同步更新 Android APK、UGOS Pro `1.3.0.0007` 双架构 UPK、SHA-256 校验文件和发布说明。
